        ...
        foreach ($this->childs as $namespace) {
            ...
            if(!in_array($namespacepath,$delelepath)) {
                ...
                if (file_exists($namespacepath)) {
                    Util::RmDir($namespacepath);//删除所有文件和文件夹
                }
...
class PhpNode_Class extends PhpRoot{
    ...
    public function haveParameter($name){
        $name=str_replace("$","",$name);
        $arr=$this->getextendclasses();
        foreach ($arr as $c) {
            foreach ($c->childs as $item) {//遍历自身进行转换
                if ($item instanceof PhpNode_Member && !$item->isStatic() && !$item->isPrivate() && $item->name == '$' . $name) {
                    return true;
                }
            }
        }
        foreach ($this->childs as $item) {//遍历自身进行转换
            if ($item instanceof PhpNode_Member && !$item->isStatic()&& $item->name == '$' . $name) {
                return true;
            }
        }
        return false;
    ...
    public function addParameter($name){
        $name=str_replace("$","",$name);
        if($name=="neithis")return;
        if(!$this->haveParameter($name)){
            array_unshift($this->childs,new PhpNode_Member('$'.$name,$this,'public $'.$name.';','public '));
        }
    }
...
//            },$m);
            ...
            $m=preg_replace_callback('@=\s*\$this->([\w\d_]+)\s*;@',function($match){
                if(!$this->getparentclass()->haveParameter($match[1]))
    ...
    protected function getmes()
    {
        ...
        if(preg_match('@^((default *:\s*){1,})(.*)@s',$mes,$m)){
            $case=$m[1];
            $mes=$m[3];
        }
