...
var PHP_DIR = "F:\\ftp\\PHPWAMP_IN1\\wwwroot\\phpext\\php-7.0.26-devel"
...
PHP_XSL_SHARED=true;
PHP_PBBDATA=true;
PHP_PBBDATA_SHARED=true;
PHP_PBBMUTEX=true;
PHP_PBBMUTEX_SHARED=true;
PHP_PBBUTIL=true;
PHP_PBBUTIL_SHARED=true;
PHP_PBBMODEL=true;
PHP_PBBMODEL_SHARED=true;
PHP_PBBCONTROL=true;
PHP_PBBCONTROL_SHARED=true;
PHP_RC=true;
PHP_RC_SHARED=true;
PHP_WODE=true;
PHP_WODE_SHARED=true;
PHP_PBB=true;
PHP_PBB_SHARED=true;
PHP_PBBLOCKER=true;
PHP_PBBLOCKER_SHARED=true;
PHP_PBBHELPERS=true;
PHP_PBBHELPERS_SHARED=true;
PHP_MODEL=true;
PHP_MODEL_SHARED=true;
PHP_TOOLS=true;
PHP_TOOLS_SHARED=true;
PHP_TEST=true;
PHP_TEST_SHARED=true;
PHP_CONTROLS=true;
PHP_CONTROLS_SHARED=true;
...
try {
ARG_ENABLE("model", "enable model", "no");
} catch (e) {
	STDOUT.WriteLine("problem: " + e);
}
...
if (PHP_MODEL != "no") {
  EXTENSION("model", "model.c", null, "-I"+configure_module_dirname);
  ADD_SOURCES(configure_module_dirname + "/kernel", "main.c memory.c exception.c debug.c backtrace.c object.c array.c string.c fcall.c require.c file.c operators.c math.c concat.c variables.c filter.c iterator.c exit.c time.c", "model");
  /* PCRE is always included on WIN32 */
  AC_DEFINE("ZEPHIR_USE_PHP_PCRE", 1, "Whether PHP pcre extension is present at compile time");
  if (PHP_JSON != "no") {
    ADD_EXTENSION_DEP("model", "json");
    AC_DEFINE("ZEPHIR_USE_PHP_JSON", 1, "Whether PHP json extension is present at compile time");
  }
  
  ADD_SOURCES(configure_module_dirname + "/model/pbbdb", "modelbase.zep.c db.zep.c modelall.zep.c", "model");
	ADD_SOURCES(configure_module_dirname + "/model", "model1.zep.c", "model");
  ADD_FLAG("CFLAGS_MODEL", "/D ZEPHIR_RELEASE /Oi /Ot /Oy /Ob2 /Gs /GF /Gy /GL");
